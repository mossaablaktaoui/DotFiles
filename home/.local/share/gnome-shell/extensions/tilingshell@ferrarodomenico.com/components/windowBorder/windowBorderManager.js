...
var WindowBorder = Me.imports.components.windowBorder.windowBorder.WindowBorder;
var SignalHandling = Me.imports.utils.signalHandling.SignalHandling;
var Settings = Me.imports.settings.settings.Settings;

var WindowBorderManager = class WindowBorderManager {
  _signals;
  _border;
  _enableScaling;
  _interfaceSettings;
  ...
  enable() {
    ...
    this._signals.connect(
      ...
      () => {
        if (Settings.ENABLE_WINDOW_BORDER) this._turnOn();else
        this._turnOff();
      }
    );
  }
  ...
  _onWindowFocused() {
    ...
    if (!this._border)
    this._border = new WindowBorder(metaWindow, this._enableScaling);else
